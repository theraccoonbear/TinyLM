// A genuinely tiny language model — character-level.
//
// This is v2's exact architecture (the small Bengio-2003-style context-
// window MLP: embed the last N tokens, concat, W1+tanh, W2+softmax) with
// exactly two changes:
//
//   1. tokenize() splits into individual CHARACTERS instead of words.
//   2. --context defaults to 1 instead of 4 — the minimum meaningful
//      window, so each prediction depends on only the single previous
//      character.
//
// Nothing else in the model needed to change. That's the actual point of
// keeping this version around: a fixed-context architecture doesn't care
// what a "token" is or how wide the window is — order-1 character
// prediction and order-4 word prediction are the *same code path*, just a
// different tokenizer and a different value of one constant. What that
// architecture can't do, no matter which of those two knobs you turn, is
// see further back than `context` tokens — which is exactly why v3 exists.
//
//   [tok(t-N) ... tok(t-1)] --embed--> concat --W1,tanh--> hidden
//                                                --W2,softmax--> P(tok t)
//
// (The project's actual first prototype was TypeScript, not Rust — see
// tiny_llm.ts at commit 414de75 in git history for that literal artifact.
// This file is a from-v2 reconstruction of the same idea, in the same
// Rust codebase as v2/v3, so the whole v1->v3 arc is one language you can
// actually diff between stages instead of a language switch on top of an
// architecture switch.)
...
/// A genuinely tiny character-level language model, trained via real backprop.
#[derive(Parser, Debug)]
...
struct Cli {
    ...
    /// Characters to generate per sample
    ...
    /// else collapses to <unk>. For real text this rarely binds at all —
    /// English prose only ever uses on the order of 70-100 distinct
    /// characters, well under any reasonable cap.
    #[arg(long, default_value_t = 3000)]
    vocab_size: usize,

    /// Context window: how many previous characters condition each
    /// prediction. 1 is the minimum meaningful value — this is what makes
    /// this an order-1 Markov chain wearing a neural net costume, same as
    /// the project's original TypeScript prototype. Try --context 4 or
    /// higher to see this same code approach v2's word-level MLP, just
    /// with characters instead of words as the unit.
...
// ---------- Tokenizer ----------
// Genuinely character-level: every Unicode scalar value (including spaces,
// punctuation, and newlines) is its own token, full stop. No word/
// punctuation-attachment heuristics needed anywhere — unlike v2's
// tokenizer, this one is a lossless, exact round-trip: detokenize(tokenize(s)) == s.
...
// output layer's cost against a Zipfian real-text vocabulary. For
// character-level text this cap essentially never binds: real corpora
// only ever use on the order of 70-100 distinct characters.
